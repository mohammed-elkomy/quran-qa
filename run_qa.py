...
from transformers import (
    ...
    set_seed, tokenization_utils_fast, BertConfig,
)
...
class DataTrainingArguments:
    ...
    pad_to_max_length: bool = field(
        default=True,  # set it to false for TPU
        metadata={
...
def main():
    ...
    if data_args.dataset is not None:
        if not data_args.dataset.endswith("py"):
            ...
            raw_datasets = load_dataset(loader_script, data_files=data_files, cache_dir=model_args.cache_dir, preprocessor=None)
    ...
    config.attention_probs_dropout_prob = .15
    config.hidden_dropout_prob = .15
    config.classifier_dropout = .3
    ...
    mylogs = open(os.path.join(training_args.output_dir, "mylogs.txt"), "w")
    ...
    def prepare_train_features(examples):
        ...
        tokenized_examples = tokenizer(
            ...
            truncation="only_second" if pad_on_right else "only_first",  # truncate only the context
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            # sample_index != i, wont be equal for long sequences
            ...
            if len(answers["answer_start"]) == 0:
                ...
                if not (offsets[token_start_index][0] <= start_char and offsets[token_end_index][1] >= end_char):
                    ...
                    assert offsets[token_start_index][0] <= start_char <= end_char <= offsets[token_end_index][1]
                    ...
                    if offsets[token_end_index + 1][1] != end_char or offsets[token_start_index - 1][0] != start_char:
                        print("=" * 50, file=mylogs)
                        print("the tokenizer can't set the boundaries", file=mylogs)
                        tokenizer_start_char = offsets[token_start_index - 1][0]
                        tokenizer_end_char = offsets[token_end_index + 1][1]
                        print(f"tokenizer:{examples[context_column_name][sample_index][tokenizer_start_char:tokenizer_end_char]}", file=mylogs)
                        print(f"label slice:{examples[context_column_name][sample_index][start_char:end_char]}", file=mylogs)
                        print(f"label:{answers}", file=mylogs)
                        print("=" * 50, file=mylogs, flush=True)
    ...
    data_collator = (
        ...
        else DataCollatorWithPadding(tokenizer, pad_to_multiple_of=8 if training_args.fp16 else None)  # this when working with tpu, to use a universal squence length
    )

    # Post-processing:
    def post_processing_function(examples, features, first_prediction, stage="eval"):
        # Post-processing: we match the start logits and end logits to answers in the original context.

        first_prediction, top_k_predictions = postprocess_qa_predictions(
            examples=examples,
            features=features,
        ...
        try:
            model_dump_path = f"{os.path.split(training_args.output_dir)[-1]}-{stage}-{training_args.seed}.dump"
        ...
        if data_args.version_2_with_negative:
            formatted_predictions = [
                {"id": k, "prediction_text": v, "no_answer_probability": 0.0} for k, v in first_prediction.items()
            ]
        else:
            formatted_predictions = []
            ...
            # formatted_predictions2 = [{"id": k, "prediction_text": v} for k, v in first_prediction.items()]

        # eval_preds is a dict with {example_id : extracted_text}
        if stage == "eval":
            # in eval we have labelled data
            references = [{"id": ex["id"], "answers": ex[answer_column_name], "passage": ex[context_column_name]} for ex in examples]
            return EvalPrediction(predictions=formatted_predictions, label_ids=references)
        elif stage == "predict":
            # making a submission file without having labels
            ntop_predictions = {}
            ...
            submission_file = model_dump_path.replace(".dump", "-submission.json")
            with open(submission_file, "w", encoding="utf-8") as f:
                f.write(json.dumps(ntop_predictions))
            print("submission file saved successfully")

            try:
                shutil.copy(submission_file, os.path.join("/gdrive/MyDrive", submission_file))
            except:
                pass

    metric_file = data_args.eval_metric
    if data_args.version_2_with_negative:
        metric_file = "squad_v2"

    metric = load_metric(metric_file)

    def compute_metrics(p: EvalPrediction):
        return metric.compute(predictions=p.predictions, references=p.label_ids, model_name_or_path=model_args.model_name_or_path)
    ...
    if training_args.do_train:
        ...
        trainer.save_metrics("train", metrics, training_args.seed)
        trainer.save_state()
    ...
    if training_args.do_eval:
        ...
        trainer.save_metrics("eval", metrics, training_args.seed)

    # Prediction
    if training_args.do_predict:
        logger.info("*** Predict ***")
        trainer.predict(predict_dataset, predict_examples)
