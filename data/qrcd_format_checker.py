'''
Feb 10, 2022
A script to check a run file for submission to the Qur'an QA 2022 shared Task with respect to:
    - utf-8 encoding
    - json file
    - structure and field names {"pq_id":[{"answer":list,"rank":int,"score":float}]}
...
def check_structure(data):
    ...
    for pq_id in data:
        ...
        if len(quest_answer_list) > 5:
            print(f"Warning: Number of answers at {pq_id} exceeds 5")
...
# rather than coming from the cmd
def functional_checker(input_path):
    try:
        if input_path[-5:] != ".json":
            print("Error: The input file is NOT a JSON file!")
        else:
